...
/*   Updated: 2024/01/07 15:03:26 by rmouchel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_process_chunk *p_chunk, int *size)
{
	int	end_of_pair;
	int	i;

	if (p_chunk->current_pair + 1 < p_chunk->total_chunks)
		end_of_pair = p_chunk->chunks[p_chunk->current_pair + 1].end;
	else
		end_of_pair = *size;
	i = 0;
	while (i < *size)
	{
		if ((*stack_a)->index >= p_chunk->chunks
			[p_chunk->current_pair].start
			&& (*stack_a)->index <= end_of_pair)
		{
			pb(stack_a, stack_b);
			if (p_chunk->current_pair + 1 < p_chunk->total_chunks
				&& (*stack_b)->index > p_chunk->chunks
				[p_chunk->current_pair].end)
				rb(stack_b);
		}
		else
			ra(stack_a);
	...
	*size -= end_of_pair - p_chunk->chunks[p_chunk->current_pair].start + 1;
}
...
int	sort_large_elements(t_stack **stack_a, t_stack **stack_b)
{
	...
	distribute_to_chunks(stack_a, stack_b, chunks, size / chunk_size);
